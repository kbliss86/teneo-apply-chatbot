...
def ActionNode = "<tags><AppMethod>Chatbot</AppMethod><AppChannel>PerDiem</AppChannel><Source>${sUTMSOURCE}</Source><Medium>${sUTMMEDIUM}</Medium><Campaign>${sUTMCAMPAIGN}</Campaign><ID>${sUTMID}</ID><Content>${sUTMCONTENT}</Content><Term>${sUTMTERM}</Term><Channel>${sUTMCHANNEL}</Channel></tags>";
